...
// Check if n can be represented as product of two ints > 1
function isCompositeProduct(n: number): { result: boolean; explanation: string } {
  if (!Number.isInteger(n) || n <= 3) {
    return {
      result: false,
      explanation: "Must be an integer greater than 3 (smallest product is 2*2).",
    };
  }
  for (let i = 2; i * i <= n; i++) {
    if (n % i === 0) {
    ...
    if (opts.command === "check" && opts.recordId) {
      ...
      output = records
        .map((r) => {
          ...
          return `${r.id} (${r.value}) → ${res.result ? "YES" : "NO"}: ${res.explanation}`;
        })
        .join("\n\n");
    } else {
        ...
        <Space wrap>
          <Tooltip title="Run check on this number">
            <Button
              ...
              onClick={() => runCommand({ recordId: record.id, command: "check" })}
          ...
          <Popconfirm
            ...
            cancelText="Cancel"
          >
            <Button danger icon={<DeleteOutlined />} aria-label={`delete-${record.id}`}>
              Delete
            </Button>
          </Popconfirm>
        </Space>
        ...
        <Row align="middle" justify="space-between">
          <Col>
            <Space align="center">
              ...
              <Text type="secondary">(check if n = a × b for integers &gt; 1)</Text>
            </Space>
          </Col>
          <Col>
            <Space>
              <Button
                ...
                onClick={() => runCommand({ command: "check:all", recordId: null })}
      ...
      <Content style={{ padding: 24 }}>
            ...
            <section aria-labelledby="create-record-heading">
              ...
              <Form form={form} layout="vertical" onFinish={handleCreate}>
                ...
                <Form.Item
                  label="Integer value"
                  name="value"
                  rules={[{ required: true, message: "Please enter an integer value" }]}
                >
                  <Input
              ...
              <div aria-live="polite">
                ...
                <ul>
                  <li>Enter an integer &gt; 1 (smallest product is 2 × 2 = 4).</li>
                  <li>
                    Use <code>Check</code> to run the command on one record, or <code>Check all</code>.
                  </li>
                </ul>
              </div>
            </section>
          ...
          <Col xs={24} md={16} lg={18}>
            ...
            <div style={{ marginTop: 12 }}>
              <Table
                ...
                locale={{ emptyText: "No records yet — create one to get started" }}
            ...
            <section aria-labelledby="commands-heading">
              ...
              <div role="region" aria-live="polite">
                {commands.length === 0 ? (
                  <Text type="secondary">No commands run yet. Run a check to see results here.</Text>
                ) : (
                  <Table
                    dataSource={commands}
                    rowKey={(c) => c.id}
                    ...
                    columns={[
                      ...
                      { title: "Command", dataIndex: "command", key: "command" },
                      {
                        ...
                        dataIndex: "recordId",
                        key: "recordId",
                        render: (rid) => (rid ? <Text code>{rid}</Text> : <Text type="secondary">global</Text>),
                      },
                      {
                        title: "Output",
                        dataIndex: "output",
                        key: "output",
                        render: (_out, rec: CommandRun) => (
                          <Button
                            onClick={() => {
                              setDrawerOpen(true);
                              setSelectedRecord(records.find((r) => r.id === rec.recordId) ?? null);
                              setCommands((s) => [rec, ...s.filter((x) => x.id !== rec.id)]);
                            }}
                          >
                            View output
                          </Button>
                      ...
                      },
                      {
                        ...
                        render: (_, rec) => (
                          <Popconfirm
                            ...
                            onConfirm={() => setCommands((s) => s.filter((c) => c.id !== rec.id))}
                          >
                            <Button danger icon={<DeleteOutlined />} aria-label={`delete-cmd-${rec.id}`} />
                          </Popconfirm>
                        ),
                      },
                    ]}
        ...
        >
          <div tabIndex={-1} ref={outputRef} style={{ outline: "none" }}>
            ...
            {commands.length > 0 ? (
              <div style={{ whiteSpace: "pre-wrap", fontFamily: "monospace" }} aria-live="polite">
                {commands[0].output}
              </div>
            ) : (
